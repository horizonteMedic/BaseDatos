---------------------------------------------------------------------------
  ...
  CREATE OR REPLACE FUNCTION obtener_informacion_sede()
RETURNS TABLE(
    dir_sede4 text, email_sede4 text, tel_sede4 text, cel_sede4 text,
    dir_sede3 text, email_sede3 text, tel_sede3 text,
    dir_sede2 text, email_sede2 text, tel_sede2 text, cel_sede2 text,
    dir_sede1 text, email_sede1 text, tel_sede1 text
) AS
$BODY$
SELECT
    (SELECT direccion FROM sede WHERE cod_sede = 4) AS dir_sede4,
    (SELECT email FROM sede WHERE cod_sede = 4) AS email_sede4,
    (SELECT telefono FROM sede WHERE cod_sede = 4) AS tel_sede4,
    (SELECT celular FROM sede WHERE cod_sede = 4) AS cel_sede4,

    (SELECT direccion FROM sede WHERE cod_sede = 3) AS dir_sede3,
    (SELECT email FROM sede WHERE cod_sede = 3) AS email_sede3,
    (SELECT telefono FROM sede WHERE cod_sede = 3) AS tel_sede3,

    (SELECT direccion FROM sede WHERE cod_sede = 2) AS dir_sede2,
    (SELECT email FROM sede WHERE cod_sede = 2) AS email_sede2,
    (SELECT telefono FROM sede WHERE cod_sede = 2) AS tel_sede2,
    (SELECT celular FROM sede WHERE cod_sede = 2) AS cel_sede2,

    (SELECT direccion FROM sede WHERE cod_sede = 1) AS dir_sede1,
    (SELECT email FROM sede WHERE cod_sede = 1) AS email_sede1,
    (SELECT telefono FROM sede WHERE cod_sede = 1) AS tel_sede1;
$BODY$
LANGUAGE sql STABLE;

CREATE TABLE consentimiento_rayosx(
  n_orden integer NOT NULL,
  fecha date,
  hora time without time zone,
  user_registro text,
  CONSTRAINT consentimientorayosx_pkey PRIMARY KEY (n_orden)
)

CREATE OR REPLACE FUNCTION obtener_reporte_consentimiento_rayosx(
    IN p_norden integer,
...
BEGIN
    ...
    CREATE TEMP TABLE temp_digitalizados_consentimiento_rayosx AS
    ...
    INNER JOIN consentimiento_rayosx crx ON crx.n_orden = noo.n_orden
    INNER JOIN sede_multisucursal sm ON noo.cod_sede = sm.id
    INNER JOIN temp_digitalizados_consentimiento_rayosx td ON crx.n_orden = td.n_orden
    WHERE noo.n_orden = p_norden;

    DROP TABLE IF EXISTS temp_digitalizados_consentimiento_rayosx;
