...
CREATE OR REPLACE FUNCTION obtener_reporte_testfatigasomnolencia(IN p_norden integer,IN name_service text)
  RETURNS TABLE(nombres text, edad integer,sexo_pa text,area_o text,razon_empresa text,
	f_examen date,n_orden integer, dni integer,sede text,cod_eval integer,rbs1nunca boolean,rbs1poca boolean,rbs1moderada boolean,rbs1alta boolean,
	rbs2nunca boolean,rbs2poca boolean,rbs2moderada boolean,rbs2alta boolean,rbs3nunca boolean,rbs3poca boolean,rbs3moderada boolean,
        rbs3alta boolean,rbs4nunca boolean,rbs4poca boolean,rbs4moderada boolean,rbs4alta boolean,rbs5nunca boolean,rbs5poca boolean,
	rbs5moderada boolean,rbs5alta boolean,rbs6nunca boolean,rbs6poca boolean,rbs6moderada boolean,rbs6alta boolean,rbs7nunca boolean,
	rbs7poca boolean,rbs7moderada boolean,rbs7alta boolean,rbs8nunca boolean,rbs8poca boolean,rbs8moderada boolean,rbs8alta boolean,
	rbs9nunca boolean,rbs9poca boolean,rbs9moderada boolean,rbs9alta boolean,rbsi boolean,rbno boolean,txtpuntaje text,txtmedico text,
	dni_user integer,user_registro text, name_jasper text, color integer
	 ) AS
$BODY$
BEGIN
  RETURN QUERY
  ...
  FROM datos_paciente dp
  INNER JOIN n_orden_ocupacional noo ON noo.cod_pa = dp.cod_pa
  INNER JOIN test_fatiga_somnolencia tfs ON tfs.n_orden = noo.n_orden
  INNER JOIN sede_multisucursal sm ON noo.cod_sede = sm.id
