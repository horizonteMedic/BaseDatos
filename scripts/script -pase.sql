...
CREATE OR REPLACE FUNCTION obtener_reporte_ficha_interconsulta(
  ...
  codigoclinica text, fecha_apertura date, edadpaciente text, celularPaciente text, frecuenciacardiaca text, sistolica text, diastolica text, 
  frecuenciarespiratoriatriaje_f_respiratoria text, imctriaje text, saturacionoxigenotriaje_sat_02 text, temperatura text, peso text, tallatriaje text, 
  codigofichainterconsulta integer, fechaexamen date, horasalida time without time zone, dniusuario integer, especialidad text, motivo text, hallazgo text, diagnostico text, tratamiento text, apto boolean, noapto boolean, nombreusuario text, apellidousuario text, cmpusuario text, visioncercasincorregirod_v_cerca_s_od text, visioncercasincorregiroi_v_cerca_s_oi text, oftalodccmologia_odcc text, oiccoftalmologia_oicc text, visionlejossincorregirod_v_lejos_s_od text, visionlejossincorregiroi_v_lejos_s_oi text, odlcoftalmologia_odlc text, oilcoftalmologia_oilc text, vcoftalmologia_vc text, vboftalmologia_vb text, rpoftalmologia_rp text, enfermedadesocularesoftalmo_e_oculares text, nombresede text, sede text, color integer, namejasper text) AS
$BODY$
BEGIN
    RETURN QUERY
    SELECT 
...
fechanacimientopaciente date, ocupacionpaciente text, 
lugarNacimientoPaciente text, nivelEstudioPaciente text,
cargopaciente text, areapaciente text, contrata text, 
...
presentacionInadecuado_rb_inadecuado boolean,
...
marchaConDificultad_rb_condificultad boolean,
marchaSinDificultad_rb_sindificultad boolean,
tiempoOrientado_rb_tiempo_orientado boolean,
tiempoDesorientado_rb_tiempo_desorientado boolean,
espacioOrientado_rb_espacio_orientado boolean,
espacioDesorientado_rb_espacio_desorientado boolean,
personaOrientado_rb_persona_orientado boolean,
personaDesorientado_rb_persona_desorientado boolean,
apto_apto boolean,
nivelIntelectual_resul_nivel_intelectual text,
coordinacionVisomotriz_resul_coordinacion_visomotriz text,
nivelMemoria_resul_nivel_memoria text,
personalidad_resul_personalidad text,
efectividad_resul_efectividad text,
areaCognitiva_areacognitiva text,
areaEmocional_areaemocional text,
recomendaciones_recomendaciones text,
...
$BODY$
  ...
  insert into config_general_service_digital (name_service,descripcion,firma_p,huella_p,sello_prof_s,sello_doc_asig,sello_doc_adic)
			values('ficha_psicologica_anexo02','formulario de ficha psicologica anexo02',false,false,true,false,false);
