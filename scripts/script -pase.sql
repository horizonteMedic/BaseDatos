...
fechaEntrevista date, areaIntelectual text, areaPersonalidad text, areaOrganicidad text,
areaPsicomotricidad text, recomendaciones text, aprobo boolean,
desaprobo boolean, codigoInforme integer,
nombresede text, sede text, color integer, 
namejasper text
) AS
$BODY$
BEGIN
	...
	INNER JOIN sede_multisucursal AS sm 
	    ON n.cod_sede = sm.id
	INNER JOIN informe_psicologico AS ip
...
$BODY$
  ...
  insert into config_general_service_digital (name_service,descripcion,firma_p,huella_p,sello_prof_s,sello_doc_asig,sello_doc_adic)
			values('informe_psicologico','formulario de informe psicologico',false,false,true,false,false);
