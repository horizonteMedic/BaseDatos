...
CREATE OR REPLACE FUNCTION obtener_reporte_certificado_aptitud_herramientas_manuales(
  ...
  idCertificado integer, apto boolean, aptoRestriccion boolean, aptoTemporal boolean, observacion text, fechaCertificado date, fechaCaducidad date, nombreMedico text,
  nombresede text, numerosede text, sede text, color integer, namejasper text) AS
$BODY$
BEGIN
    RETURN QUERY
    SELECT 
...
insert into config_general_service_digital (name_service,descripcion,firma_p,huella_p,sello_prof_s,sello_doc_asig,sello_doc_adic)
			values('certificado_aptitud_herramientas_manuales','formulario de certificado aptitud herramientas manuales',true,true,true,false,false);
