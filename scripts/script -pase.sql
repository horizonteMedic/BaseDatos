SELECT n_orden
FROM n_orden_ocupacional LIMIT 1;
...
SELECT d.cod_pa,
       ...
       (SELECT s.nombre_sede FROM sede s WHERE s.cod_sede = n.cod_sede)::text AS nombresede, (SELECT CAST(sm.descripcion AS TEXT)
...
RETURN QUERY
SELECT d.cod_pa,
       d.nombres_pa::text                                                                        AS nombre, d.apellidos_pa,
       d.direccion_pa,
       d.sexo_pa,
       d.fecha_nacimiento_pa,
...
RETURN QUERY
SELECT d.cod_pa,
       d.nombres_pa::text, d.apellidos_pa::text, d.direccion_pa::text, d.sexo_pa,
       d.fecha_nacimiento_pa,
       d.ocupacion_pa::text, d.lugar_nac_pa::text, d.nivel_est_pa::text, d.estado_civil_pa::text, n.cargo_de::text, n.area_o::text, n.razon_contrata::text, n.n_orden,
       n.razon_empresa::text, n.cod_clinica::text, n.nom_examen::text, CAST(obtener_edad(d.fecha_nacimiento_pa, current_date) AS TEXT),
       -- Resultados
...
SELECT d.cod_pa,
       d.cod_pa as dni,
       (d.nombres_pa || ' ' || d.apellidos_pa)::text, d.sexo_pa::text, d.fecha_nacimiento_pa,
       d.lugar_nac_pa::text, d.estado_civil_pa::text, d.nivel_est_pa::text, CAST(obtener_edad(d.fecha_nacimiento_pa, current_date) AS TEXT) as edad,
       d.ocupacion_pa::text, n.n_orden,
       n.nom_examen::text, n.razon_empresa,
       n.razon_contrata::text, n.cargo_de,
       n.area_o::text, n.fecha_apertura_po,
...
RETURN QUERY
SELECT d.cod_pa,
       d.nombres_pa::text, d.apellidos_pa::text, d.direccion_pa::text, d.sexo_pa::text, d.fecha_nacimiento_pa,
       ...
       n.n_orden::text AS cod_clinica, n.nom_examen::text AS tipo_examen, f.criterio_inteligencia,
       f.criterio_fobias,
...
SELECT d.nombres_pa || ' ' || d.apellidos_pa,
       ...
       ab.n_orden,
       (SELECT nombre_sede FROM sede WHERE cod_sede = n.cod_sede)::text, ab.cod_ab,
       ab.txtcreatinina,
       ab.txtcolesterol,
       ab.txtldlcolesterol,
       ab.txthdlcolesterol,
...
CREATE
OR REPLACE FUNCTION obtener_name_jasper(norden_param bigint, name_service_param text) RETURNS text AS $BODY$ DECLARE
...
WHERE n_orden = norden_param;
IF
name_service_param = 'con_panel10D' THEN resultado := 'Consentimiento_Panel10D_Digitalizado'; ELSIF
name_service_param = 'con_panel5D' THEN IF name_empresa_busqueda_var = 'OBRASCÓN HUARTE LAIN S.A' THEN resultado := 'Consentimiento_Panel5D_ohla_Digitalizado';
ELSE resultado := 'Consentimiento_Panel5D_Digitalizado';
END IF; ELSIF
name_service_param = 'con_panel3D' THEN resultado := 'Consentimiento_Panel3D_Digitalizado'; ELSIF
name_service_param = 'con_panel2D' THEN resultado := 'Consentimiento_Panel2D_Digitalizado'; ELSIF
name_service_param = 'consent_Muestra_Sangre' THEN resultado := 'Consentimiento_Muestra_Sangre_Digitalizado'; ELSIF
name_service_param = 'consent_marihuana' THEN resultado := 'Consentimiento_Marihuana_Digitalizado'; ELSIF
name_service_param = 'consent_Boro' THEN resultado := 'Consentimiento_Boro_Digitalizado';  ELSIF
...
END IF; ELSIF
name_service_param = 'lab_clinico' THEN resultado := 'LaboratorioClinico_Digitalizado'; ELSIF
name_service_param = 'hemograma_autom' THEN resultado := 'Hematologia_Digitalizado'; ELSIF
name_service_param = 'lgonadotropina' THEN resultado := 'LGonadotropina_Digitalizado'; ELSIF
name_service_param = 'panel2d' THEN resultado := 'Panel2d_Digitalizado'; ELSIF
name_service_param = 'panel3d' THEN resultado := 'Panel3d_Digitalizado'; ELSIF
name_service_param = 'toxicologia' THEN IF name_empresa_busqueda_var = 'OBRASCÓN HUARTE LAIN S.A' THEN resultado := 'ResultadosPanel5d_ohla_Digitalizado';
ELSE resultado := 'ResultadosPanel5d_Digitalizado';
END IF; ELSIF
name_service_param = 'panel10d' THEN resultado := 'Panel10d_Digitalizado'; ELSIF
name_service_param = 'inmunologia' THEN resultado := 'InmunologiaLab_Digitalizado'; ELSIF
name_service_param = 'microbiologia' THEN IF name_valor_microbiologia_var IS NOT NULL
...
END IF; ELSIF
name_service_param = 'lhepatitis' THEN IF name_valor_hepatitisa_var IS NOT NULL
AND LENGTH(name_valor_hepatitisa_var) > 0 THEN resultado := 'LHepatitisA_Digitalizado'; ELSIF
...
END IF; ELSIF
name_service_param = 'l_bioquimica' THEN resultado := 'LBioquimica_Digitalizado'; ELSIF
name_service_param = 'ac_bioquimica2022' THEN resultado := 'AnalisisClinicosB_Digitalizado'; ELSIF
name_service_param = 'perfil_hepatico' THEN resultado := 'PerfilHepatico_Digitalizado'; ELSIF
name_service_param = 'ac_coprocultivo' THEN resultado := 'coprocultivo_digitalizado'; ELSIF
name_service_param = 'ac_coproparasitologico' THEN IF valor_coproparasitologico_var = true THEN resultado := 'Coproparasitologico_Digitalizado';
ELSE resultado := 'ParasitologiaSeriado_Digitalizado';
END IF; ELSIF
name_service_param = 'examen_inmunologico' THEN IF valor_cuantitativo_antigeno_var = true THEN resultado := 'pcuantiantigeno';
ELSE IF valor_formato_marsa_var = true THEN resultado := 'pcualitativaantigenoMarsa';
ELSE resultado := 'pcualitativaantigeno';
END IF;
END IF; ELSIF
name_service_param = 'audiometria_2023' THEN IF name_empresa_busqueda_var LIKE '%MINERA BOROO MISQUICHILCA S.A.%' OR name_empresa_busqueda_var ILIKE '%EMPRESA DE TRANSPORTES N & V S.A.C.%' OR name_empresa_busqueda_var ILIKE '%DEYFOR EIRL%' THEN resultado := 'Audiometria2021-_Digitalizado_boro';
ELSE resultado := 'Audiometria2021-_Digitalizado';
END IF; ELSIF
name_service_param = 'historia_oc_info' THEN IF name_empresa_busqueda_var LIKE '%MINERA BOROO MISQUICHILCA S.A.%' OR name_empresa_busqueda_var ILIKE '%EMPRESA DE TRANSPORTES N & V S.A.C.%' OR name_empresa_busqueda_var ILIKE '%DEYFOR EIRL%' THEN resultado := 'HistoriaOcupacional_Digitalizado_boro';
ELSE resultado := 'HistoriaOcupacional_Digitalizado';
END IF; ELSIF
name_service_param = 'audiometria_po' THEN resultado := 'FichaAudiologica_Digitalizado'; ELSIF
name_service_param = 'cuestionario_audiometria' THEN resultado := 'CuestionarioAudiometria_Digitalizado'; ELSIF
name_service_param = 'oftalmologia_lo' THEN resultado := 'OftalmologiaLO'; ELSIF
name_service_param = 'oftalmologia' THEN resultado := 'Oftalmologia'; ELSIF
name_service_param = 'oftalmologia_reporte' THEN resultado := 'ReporteOftalmologico'; ELSIF
name_service_param = 'oftalmologia2021' THEN IF name_empresa_busqueda_var LIKE '%MINERA BOROO MISQUICHILCA S.A.%' OR name_empresa_busqueda_var ILIKE '%EMPRESA DE TRANSPORTES N & V S.A.C.%' OR name_empresa_busqueda_var ILIKE '%DEYFOR EIRL%' THEN resultado := 'EvaluacionOftalmologica2021_Digitalizado_boro'; ELSIF
name_empresa_busqueda_var LIKE 'OBRASCÓN HUARTE LAIN S.A' THEN resultado := 'EvaluacionOftalmologica2021_Digitalizado_ohla';
ELSE resultado := 'EvaluacionOftalmologica2021_Digitalizado';
END IF; ELSIF
name_service_param = 'odontograma' THEN resultado := 'Odontograma_Digitalizado'; ELSIF
name_service_param = 'odontograma_lo' THEN resultado := 'Odontograma_lo_Digitalizado'; ELSIF
name_service_param = 'radiografia_torax' THEN resultado := 'RagiografiaToraxPA_Digitalizado'; ELSIF
name_service_param = 'radiografia_fechas' THEN resultado := 'ReporteFechasRadiografia_Digitalizado'; ELSIF
name_service_param = 'radiografia' THEN resultado := 'RAYOSXXXOFI_Digitalizado'; ELSIF
name_service_param = 'oit' THEN IF name_empresa_busqueda_var LIKE '%MINERA BOROO MISQUICHILCA S.A.%' OR name_empresa_busqueda_var ILIKE '%EMPRESA DE TRANSPORTES N & V S.A.C.%' OR name_empresa_busqueda_var ILIKE '%DEYFOR EIRL%' THEN resultado := 'OIT_Digitalizado_boro';
ELSE resultado := 'OIT_Digitalizado';
END IF; ELSIF
name_service_param = 'evaluacion_musculo_esqueletica' THEN resultado := 'EvaluacionMuscoloEsqueletica'; ELSIF
name_service_param = 'evaluacion_musculo_esqueletica2021' THEN resultado := 'EvaluacionMuscoloEsqueletica2021_Digitalizado_boro'; ELSIF
name_service_param = 'consentimientoInformado' THEN resultado := 'conInformadoOcupacional_Digitalizado'; ELSIF
name_service_param = 'cuestionario_nordico' THEN resultado := 'CuestionarioNordico'; ELSIF
name_service_param = 'consentimiento_rayosx' THEN resultado := 'ConsentimientoRayosX_Digitalizado'; ELSIF
name_service_param = 'test_fatiga_somnolencia' THEN resultado := 'TestFatigaSomnolenia_Digitalizado_boro'; ELSIF
name_service_param = 'informe_electrocardiograma' THEN IF valor_electro_cardiograma = TRUE THEN resultado := 'InformeElectrocardiograma2023';
ELSE resultado := 'InformeElectrocardiograma_Digitalizado';
END IF; ELSIF
name_service_param = 'antece_enfermedades_altura' THEN IF name_empresa_busqueda_var LIKE '%MINERA BOROO MISQUICHILCA S.A.%' OR name_empresa_busqueda_var ILIKE '%EMPRESA DE TRANSPORTES N & V S.A.C.%' OR name_empresa_busqueda_var ILIKE '%DEYFOR EIRL%' THEN resultado := 'AnexoCB_boro_Digitalizado';
ELSE resultado := 'AnexoCB_Digitalizado';
END IF; ELSIF
name_service_param = 'anexo_agroindustrial' THEN resultado := 'Anexo2'; ELSIF
name_service_param = 'consentimientobuenasalud' THEN resultado := 'ConsentimientoBuenaSalud2021_Digitalizado'; ELSIF
name_service_param = 'anexo7c' THEN resultado := 'Anexo7C_Boro'; ELSIF
name_service_param = 'anexo16a' THEN IF name_empresa_busqueda_var LIKE '%MINERA BOROO MISQUICHILCA S.A.%' OR name_empresa_busqueda_var ILIKE '%EMPRESA DE TRANSPORTES N & V S.A.C.%' OR name_empresa_busqueda_var ILIKE '%DEYFOR EIRL%' THEN resultado := 'Anexo16ABoro_Digitalizado';
ELSE resultado := 'Anexo16A_Digitalizado';
END IF; ELSIF
name_service_param = 'antecedentes_patologicos' THEN IF name_empresa_busqueda_var LIKE '%MINERA BOROO MISQUICHILCA S.A.%' OR name_empresa_busqueda_var ILIKE '%EMPRESA DE TRANSPORTES N & V S.A.C.%' OR name_empresa_busqueda_var ILIKE '%DEYFOR EIRL%' THEN resultado := 'ficha_antecedente_patologico_boro';
ELSE resultado := 'ficha_antecedente_patologico_Digitalizado';
END IF; ELSIF
name_service_param = 'aptitud_medico_ocupacional_agro' THEN resultado := 'Aptitud_Agroindustrial'; ELSIF
name_service_param = 'certificado_aptitud_medico_ocupacional' THEN IF name_empresa_busqueda_var LIKE 'OBRASCÓN HUARTE LAIN S.A' THEN resultado := 'Aptitud_medico_ocupacional_11';
ELSE resultado := 'Aptitud_medico_ocupacional_F';
END IF; ELSIF
name_service_param = 'ficha_sas' THEN resultado := 'FichaDetencionSAS_boro_Digitalizado'; ELSIF
name_service_param = 'certificado_aptitud_medico_resumen' THEN resultado := 'Aptitud_medico_resumen_Digitalizado'; ELSIF
name_service_param = 'b_certificado_conduccion' THEN resultado := 'certificaciondeconduccion_Digitalizado_boro'; ELSIF
name_service_param = 'ficha_interconsulta' THEN resultado := 'Ficha_interconsulta_Digitalizado'; ELSIF
name_service_param = 'b_certificado_altura' THEN resultado := 'Certificacion_suficiencia_trabajos_en_altura_boro_Digitalizado'; ELSIF
name_service_param = 'informe_psicologico' THEN resultado := 'InformePsicologico_Digitalizado'; ELSIF
name_service_param = 'b_uso_respiradores' THEN resultado := 'UsoRespiradores'; ELSIF
name_service_param = 'ficha_psicologica_anexo02' THEN resultado := 'InformePsicologico_Anexo02_Digitalizado'; ELSIF
name_service_param = 'ficha_psicologica_anexo03' THEN resultado := 'FichaPsicologicaOcupacional_Digitalizado'; ELSIF
name_service_param = 'certificado_altura_poderosa' THEN resultado := 'CertificadoAlturaPoderosa_Digitalizado'; ELSIF
name_service_param = 'hoja_consulta_externa' THEN resultado := 'Hoja_Consulta_Externa'; ELSIF
name_service_param = 'aptitud_altura_poderosa' THEN resultado := 'Aptitud_Poderosa_Digitalizado'; ELSIF
name_service_param = 'aptitud_trabajos_encaliente' THEN resultado := 'Aptitud_Trabajos_EnCaliente_Digitalizado'; ELSIF
name_service_param = 'aptitud_licencia_conduciri' THEN resultado := 'Aptitud_Licencia_Conducir_Interna_Digitalizado'; ELSIF
name_service_param = 'certificado_aptitud_herramientas_manuales' THEN resultado := 'Certificado_Aptitud_Herramientas_Manuales_Digitalizado'; ELSIF
name_service_param = 'informe_psicolaboral' THEN resultado := 'Informe_PsicolaboralBoroo_Digitalizado'; ELSIF
name_service_param = 'certificado_manipuladores_barrick' THEN resultado := 'CertificadoMedicoManipuladores_Barrick_Digitalizado'; ELSIF
name_service_param = 'informe_psicologico_estres' THEN resultado := 'InformePsicologicoAdecoEstres_Digitalizado'; ELSIF
name_service_param = 'evaluacion_psicologica_poderosa' THEN resultado := 'InformePsicologico_Digitalizado'; ELSIF
name_service_param = 'psicologia_espacios_confinados' THEN resultado := 'formatPsicologia_SuficienciaEspaciosC'; ELSIF
name_service_param = 'informe_riesgos_psicosociales' THEN resultado := 'Informe_Riesgos_Psicosociales_Digitalizado'; ELSIF
name_service_param = 'informe_burnout' THEN resultado := 'Informe_burnout_Digitalizado'; ELSIF
name_service_param = 'psicologiafobias' THEN resultado := 'formatPsicologia_Digitalizado'; ELSIF
name_service_param = 'calidad_sueño' THEN resultado := 'CUESTIONARIO_CALIDAD_DE_SUEÑO_Digitalizado'; ELSIF
name_service_param = 'trastornos_personalidad' THEN resultado := 'Informe_Psico_Test_Personalidad'; ELSIF
name_service_param = 'infor_conductores' THEN resultado := 'Informe_Psico_Conductores'; ELSIF
name_service_param = 'certificacion_medica_altura' THEN resultado := 'A_CertificacionMedicaPTA_Digitalizado'; ELSIF
name_service_param = 'alto_riesgo' THEN resultado := 'Informe_Psico_Alto_Riesgo'; ELSIF
name_service_param = 'especificos' THEN resultado := 'Informe_Psico_Especificos'; ELSIF
name_service_param = 'cuestionario_berlin' THEN resultado := 'Informe_Psico_Cuestionario_Berlin'; ELSIF
name_service_param = 'exam_complementarios' THEN resultado := 'Informe_Psico_Exam_Complementario'; ELSIF
name_service_param = 'psi_brigadistas' THEN resultado := 'Informe_Psico_Brigadista'; ELSIF
name_service_param = 'bombaelectrica' THEN resultado := 'Informe_Psico_BombaElectrica'; ELSIF
name_service_param = 'cuadradorvigia' THEN resultado := 'Informe_Psico_CuadradorVigia'; ELSIF
name_service_param = 'riesgocoronario' THEN resultado := 'Informe_Lab_Riesgo_Coronario';ELSIF
name_service_param = 'fobias' THEN resultado := 'Informe_Psico_Fobias'; ELSIF
name_service_param = 'aversionalriesgo' THEN resultado := 'Informe_Psico_AversioRiesgo'; ELSIF
UPPER(name_service_param) = 'CONSENT_SINTOMATICO' THEN resultado := 'Consent_Admision_DeclaracionSintomaticoRespiratorio';
    ELSIF
...
WHERE n.n_orden = p_norden LIMIT 1;
END; $BODY$
LANGUAGE plpgsql;

CREATE
OR REPLACE FUNCTION sp_validar_existencia_servicios( IN p_historia_clinica bigint, IN p_examen_med text ) RETURNS TABLE(id_resp integer, mensaje text) AS $BODY$ declare
...
if
(v_id_existencia = 0) THEN v_mensaje := 'SIN REGISTROS EN EL SISTEMA';
...
WHERE n_orden = p_historia_clinica
  AND name_conset = p_examen_med LIMIT 1;
if
(v_id_existencia = 0) THEN v_mensaje := 'SIN REGISTROS EN EL SISTEMA';
else v_mensaje := 'YA FUE REGISTRADO';
end if;
...
WHERE n_orden = p_historia_clinica LIMIT 1;
if
(v_triaje_existencia_espirometria = 1) THEN if(v_id_existencia = 0) THEN v_mensaje := 'SIN REGISTROS EN EL SISTEMA';
else v_mensaje := 'YA FUE REGISTRADO';
end if;
else v_mensaje := 'DEBE PASAR POR TRIAJE PRIMERO (OBLIGATORIO)'; v_id_existencia
:= 2;
end if;
end if; if
...
IF
(v_agudeza_visual_existencia = 1) THEN IF(v_id_existencia = 0) THEN v_mensaje := 'SIN REGISTROS EN EL SISTEMA';
else v_mensaje := 'YA FUE REGISTRADO';
end if;
else v_mensaje := 'DEBE PASAR POR AGUDEZA VISUAL PRIMERO (OBLIGATORIO)'; v_id_existencia
:= 2;
end if;
end if; if
...
WHERE n_orden = p_historia_clinica LIMIT 1;
if
(v_tabla_necesaria_existencia = 1) THEN if(v_id_existencia = 0) THEN v_mensaje := 'SIN REGISTROS EN EL SISTEMA';
else v_mensaje := 'YA FUE REGISTRADO';
end if;
else v_mensaje := 'DEBE PASAR POR ANEXO 2 PRIMERO (OBLIGATORIO)'; v_id_existencia
:= 2;
end if;
end if; if
...
else v_mensaje := 'DEBE PASAR POR ANEXO 16 PRIMERO (OBLIGATORIO)'; v_id_existencia
:= 2;
end if;
end if; if
...
else v_mensaje := 'DEBE PASAR POR FICHA SAS PRIMERO (OBLIGATORIO)'; v_id_existencia
:= 2;
end if;
end if; if
...
else v_mensaje := 'DEBE PASAR POR AUDIOMETRIA PRIMERO (OBLIGATORIO)'; v_id_existencia
:= 2;
end if;
end if; if
...
CREATE
OR REPLACE FUNCTION obtener_parametros_digitalizados( IN norden_param bigint, IN name_servicio_param text ) RETURNS TABLE( descripcion text, name_digitalizacion text, dni integer ) AS $BODY$ DECLARE
dni_paciente_var INTEGER; dni_user_registro_var
INTEGER; dni_user_doctor_asig_var
INTEGER; dni_user_doc_adic_var
INTEGER; empresa_var
TEXT; user_registro_var
TEXT; sede_var
INTEGER; completo_electro_var
BOOLEAN;
...
-- Primera condición
IF
name_servicio_param = 'con_panel10D' THEN IF(
SELECT  firma_p
FROM config_general_service_digital
WHERE name_service = name_servicio_param ) THEN descripcion := 'FIRMA DEL PACIENTE'; name_digitalizacion
:= 'FIRMAP'; dni
:= dni_paciente_var; RETURN
NEXT;
END IF; IF
(
...
-- Segunda condición
 IF
name_servicio_param = 'con_panel5D' THEN IF(
SELECT  firma_p
FROM config_general_service_digital
WHERE name_service = name_servicio_param ) THEN descripcion := 'FIRMA DEL PACIENTE'; name_digitalizacion
:= 'FIRMAP'; dni
...
FROM config_general_service_digital
WHERE name_service = name_servicio_param ) AND empresa_var = 'OBRASCÓN HUARTE LAIN S.A' ) THEN IF(empresa_var = 'OBRASCÓN HUARTE LAIN S.A') THEN dni_user_registro_var := 42664426;
ELSE dni_user_registro_var := 1;
end if; descripcion
:= 'SELLO DEL MEDICO OCUPACIONAL ASIGNADO'; name_digitalizacion
:= 'SELLOFIRMADOCASIG'; dni
:= dni_user_registro_var; RETURN
NEXT;
END IF;
END IF;
-- Tercera condicion
 IF
name_servicio_param = 'con_panel3D' THEN IF(
SELECT  firma_p
FROM config_general_service_digital
WHERE name_service = name_servicio_param ) THEN descripcion := 'FIRMA DEL PACIENTE'; name_digitalizacion
:= 'FIRMAP'; dni
...
-- Cuarta condición
 IF
name_servicio_param = 'con_panel2D' THEN IF(
SELECT  firma_p
FROM config_general_service_digital
WHERE name_service = name_servicio_param ) THEN descripcion := 'FIRMA DEL PACIENTE'; name_digitalizacion
:= 'FIRMAP'; dni
...
-- Quinta condición
 IF
name_servicio_param = 'consent_Muestra_Sangre' THEN IF(
SELECT  firma_p
FROM config_general_service_digital
WHERE name_service = name_servicio_param ) THEN descripcion := 'FIRMA DEL PACIENTE'; name_digitalizacion
:= 'FIRMAP'; dni
...
descripcion
:= 'SELLO DEL PROFESIONAL DE SALUD'; name_digitalizacion
:= 'SELLOFIRMA'; dni
:= dni_user_registro_var; RETURN
NEXT;
END IF;
...
-- Sexta condición
 IF
name_servicio_param = 'consent_marihuana' THEN IF(
SELECT  firma_p
FROM config_general_service_digital
WHERE name_service = name_servicio_param ) THEN descripcion := 'FIRMA DEL PACIENTE'; name_digitalizacion
:= 'FIRMAP'; dni
...
-- Septima condición
 IF
name_servicio_param = 'consent_Boro' THEN IF(
SELECT  firma_p
FROM config_general_service_digital
WHERE name_service = name_servicio_param ) THEN descripcion := 'FIRMA DEL PACIENTE'; name_digitalizacion
:= 'FIRMAP'; dni
...
END IF;
END IF; IF
name_servicio_param = 'analisis_bioquimicos' THEN IF(
...
SELECT sello_doc_asig
FROM config_general_service_digital
WHERE name_service = name_servicio_param ) THEN IF empresa_var = 'OBRASCÓN HUARTE LAIN S.A' THEN dni_user_registro_var := 42664426;
ELSIF
empresa_var = 'MONARCA GOLD S.A.C.' THEN dni_user_registro_var := 66666666;
ELSE dni_user_registro_var := 1;
END IF; descripcion
:= 'SELLO DEL MEDICO OCUPACIONAL ASIGNADO'; name_digitalizacion
:= 'SELLOFIRMADOCASIG'; dni
...
END IF;
END IF; IF
name_servicio_param = 'lab_clinico' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'hemograma_autom' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'lgonadotropina' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'panel2d' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'panel3d' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'toxicologia' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'panel10d' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'inmunologia' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'microbiologia' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'lhepatitis' THEN IF(
...
-- perfil renal
 IF
name_servicio_param = 'l_bioquimica' THEN IF(
SELECT  sello_prof_s
FROM config_general_service_digital
WHERE name_service = name_servicio_param ) THEN
SELECT user_registro
...
-- acido urino
 IF
name_servicio_param = 'ac_bioquimica2022' THEN IF(
SELECT  sello_prof_s
FROM config_general_service_digital
WHERE name_service = name_servicio_param ) THEN
SELECT user_registro
...
-- perfil hepatico
 IF
name_servicio_param = 'perfil_hepatico' THEN IF(
SELECT  sello_prof_s
FROM config_general_service_digital
WHERE name_service = name_servicio_param ) THEN
SELECT user_registro
...
-- coprocultivo
 IF
name_servicio_param = 'ac_coprocultivo' THEN IF(
SELECT  sello_prof_s
FROM config_general_service_digital
WHERE name_service = name_servicio_param ) THEN
SELECT user_registro
...
-- coproparasitologico
 IF
name_servicio_param = 'ac_coproparasitologico' THEN IF(
SELECT  sello_prof_s
FROM config_general_service_digital
WHERE name_service = name_servicio_param ) THEN
SELECT user_registro
...
-- examen inmunologico
 IF
name_servicio_param = 'examen_inmunologico' THEN IF(
SELECT  firma_p
FROM config_general_service_digital
WHERE name_service = name_servicio_param ) THEN descripcion := 'FIRMA DEL PACIENTE'; name_digitalizacion
:= 'FIRMAP'; dni
...
END IF;
END IF; IF
name_servicio_param = 'audiometria_2023' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'historia_oc_info' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'audiometria_po' THEN IF(
...
WHERE UPPER(usuario_user) = UPPER(user_registro_var);
IF
empresa_var = 'OBRASCÓN HUARTE LAIN S.A' THEN dni_user_registro_var := 74723311;
END IF; descripcion
:= 'SELLO DEL PROFESIONAL DE SALUD'; name_digitalizacion
:= 'SELLOFIRMA'; dni
:= dni_user_registro_var; RETURN
...
FROM config_general_service_digital
WHERE name_service = name_servicio_param ) THEN IF(empresa_var = 'OBRASCÓN HUARTE LAIN S.A') THEN dni_user_registro_var := 55555555;
ELSE dni_user_registro_var := 1;
end if; descripcion
:= 'SELLO DEL DOCTOR ADICIONAL'; name_digitalizacion
...
END IF;
END IF; IF
name_servicio_param = 'cuestionario_audiometria' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'oftalmologia' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'oftalmologia_lo' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'oftalmologia2021' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'odontograma' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'odontograma_lo' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'radiografia_torax' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'radiografia' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'oit' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'evaluacion_musculo_esqueletica' THEN IF(
...
WHERE UPPER(usuario_user) = UPPER(user_registro_var);
IF
empresa_var = 'OBRASCÓN HUARTE LAIN S.A' THEN dni_user_registro_var := 42664426; ELSIF
empresa_var = 'MONARCA GOLD S.A.C.' THEN dni_user_registro_var := 66666666;
END IF; descripcion
:= 'SELLO DEL PROFESIONAL DE SALUD'; name_digitalizacion
:= 'SELLOFIRMA'; dni
...
END IF;
END IF; IF
name_servicio_param = 'evaluacion_musculo_esqueletica2021' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'consentimientoInformado' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'cuestionario_nordico' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'consentimiento_rayosx' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'test_fatiga_somnolencia' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'informe_electrocardiograma' THEN IF(
...
:= 'SELLO DEL PROFESIONAL DE SALUD'; name_digitalizacion
:= 'SELLOFIRMA'; IF
completo_electro_var = TRUE THEN dni := 70436528;
ELSE dni := dni_user_registro_var;
END IF; RETURN
NEXT;
END IF;
END IF; IF
name_servicio_param = 'antece_enfermedades_altura' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'anexo_agroindustrial' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'consentimientobuenasalud' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'anexo7c' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'anexo16a' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'antecedentes_patologicos' THEN IF(
...
WHERE n_orden = norden_param;
IF
sede_var = 3 THEN dni_user_registro_var = 88888888;
...
FROM usuarios
WHERE UPPER(usuario_user) = UPPER(user_registro_var); descripcion
:= 'SELLO DEL PROFESIONAL DE SALUD'; name_digitalizacion
:= 'SELLOFIRMA'; dni
:= dni_user_registro_var; RETURN
NEXT;
...
END IF;
END IF; IF
name_servicio_param = 'ficha_sas' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'certificado_aptitud_medico_resumen' THEN IF(
...
IF
sede_var = 3 THEN dni_user_registro_var = 88888888;
ELSE IF empresa_var = 'OBRASCÓN HUARTE LAIN S.A' THEN dni_user_registro_var := 88888888; ELSIF
empresa_var = 'MONARCA GOLD S.A.C.' THEN dni_user_registro_var := 66666666;
END IF;
END IF; descripcion
:= 'SELLO DEL PROFESIONAL DE SALUD'; name_digitalizacion
...
END IF;
END IF; IF
name_servicio_param = 'b_certificado_conduccion' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'ficha_interconsulta' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'b_certificado_altura' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'informe_psicologico' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'b_uso_respiradores' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'ficha_psicologica_anexo02' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'ficha_psicologica_anexo03' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'certificado_altura_poderosa' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'aptitud_altura_poderosa' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'aptitud_trabajos_encaliente' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'aptitud_licencia_conduciri' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'hoja_consulta_externa' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'certificado_aptitud_herramientas_manuales' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'resumen_medico_poderosa' THEN IF(
...
WHERE UPPER(usuario_user) = UPPER(user_registro_var);
IF
empresa_var = 'OBRASCÓN HUARTE LAIN S.A' THEN dni_user_registro_var := 88888888;
END IF; descripcion
:= 'SELLO DEL PROFESIONAL DE SALUD'; name_digitalizacion
:= 'SELLOFIRMA'; dni
:= dni_user_registro_var; RETURN
...
END IF;
END IF; IF
name_servicio_param = 'informe_psicolaboral' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'certificado_manipuladores_barrick' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'informe_psicologico_estres' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'evaluacion_psicologica_poderosa' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'psicologia_espacios_confinados' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'informe_riesgos_psicosociales' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'informe_burnout' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'psicologiafobias' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'trastornos_personalidad' THEN IF(
...
WHERE UPPER(usuario_user) = UPPER(user_registro_var);
IF
empresa_var = 'OBRASCÓN HUARTE LAIN S.A' THEN dni_user_registro_var := 42664426;
END IF; descripcion
:= 'SELLO DEL PROFESIONAL DE SALUD'; name_digitalizacion
:= 'SELLOFIRMA'; dni
:= dni_user_registro_var; RETURN
...
END IF;
END IF; IF
name_servicio_param = 'infor_conductores' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'alto_riesgo' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'certificacion_medica_altura' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'especificos' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'cuestionario_berlin' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'exam_complementarios' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'psi_brigadistas' THEN IF(
...
END IF;
END IF; IF
name_servicio_param = 'bombaelectrica' THEN IF(
...
name_service_param = 'consent_marihuana' THEN resultado := 'Consentimiento_Marihuana_Digitalizado'; ELSIF
name_service_param = 'consent_Boro' THEN resultado := 'Consentimiento_Boro_Digitalizado'; ELSIF
name_service_param = 'analisis_bioquimicos' THEN resultado := 'AnalisisBioquimicos_Digitalizado'; ELSIF
name_service_param = 'lab_clinico' THEN resultado := 'LaboratorioClinico_Digitalizado'; ELSIF
name_service_param = 'hemograma_autom' THEN resultado := 'Hematologia_Digitalizado'; ELSIF
name_service_param = 'lgonadotropina' THEN resultado := 'LGonadotropina_Digitalizado'; ELSIF
name_service_param = 'panel2d' THEN resultado := 'Panel2d_Digitalizado'; ELSIF
...
name_service_param = 'alto_riesgo' THEN resultado := 'Informe_Psico_Alto_Riesgo'; ELSIF
name_service_param = 'especificos' THEN resultado := 'Informe_Psico_Especificos'; ELSIF
name_service_param = 'cuestionario_berlin' THEN resultado := 'Informe_Psico_Cuestionario_Berlin';
END IF;
RETURN resultado;
END; $BODY$
...
CREATE
OR REPLACE FUNCTION obtener_existencias_examenes(nOrden INTEGER) RETURNS TABLE (existe BOOLEAN, nameService TEXT) LANGUAGE plpgsql AS $$
BEGIN
RETURN QUERY
...
CREATE
OR REPLACE FUNCTION listado_examenes_protocolos_paciente( protocolo_param text, historia_clinica_param integer ) RETURNS TABLE( id_archivos bigint, nombre text, ruta text, valor boolean ) LANGUAGE 'plpgsql' COST 100 VOLATILE PARALLEL UNSAFE ROWS 1000 AS $BODY$
BEGIN
RETURN QUERY
...
CREATE
OR REPLACE FUNCTION listado_pacientes_protocolo_busqueda_filtros( razon_empresa_param text, razon_contrata_param text, name_protocolo_param text, fecha_inicio_param date, fecha_fin_param date ) RETURNS TABLE( protocolos text, nombres text, dni integer, n_orden integer, razon_empresa text, razon_contrata text, tipoexamen text, fechaexamen date ) LANGUAGE 'plpgsql' AS $BODY$
BEGIN
RETURN QUERY
SELECT n.protocolo
...
IF
(validar_empresa_hm > 0) THEN RETURN QUERY
...
CREATE
OR REPLACE FUNCTION public.listado_empleado_por_username(user_name text) RETURNS TABLE( id_empleado bigint, apellidos character varying, cargo character varying, celular character varying, cip character varying, correo_elect character varying, direccion character varying, estado boolean, fecha_actualizacion date, fecha_nacimiento date, fecha_registro date, nombres character varying, num_documento bigint, tel_fijo character varying, tipo_doc character varying, ubigeo character varying, user_actualizacion character varying, user_registro character varying, sexo character varying ) LANGUAGE 'plpgsql' COST 100 VOLATILE PARALLEL UNSAFE ROWS 1000 AS $BODY$ DECLARE
validar_empresa_hm BIGINT;
BEGIN
