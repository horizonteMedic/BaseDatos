...
ALTER TABLE trastornos_personalidad
add column usuario_firma text
ALTER TABLE infor_conductores
add column usuario_firma text;
ALTER TABLE alto_riesgo
add column usuario_firma text;
...
DELETE CASCADE,
    ...
    observacion text,
    recomenda text,
    cumple_perfil boolean -- CAMPOS DE AUDITORÍA Y FIRMA AGREGADOS 
,
    user_registro text,
    usuario_firma text,
    fecha_registro date DEFAULT CURRENT_DATE
...
CREATE OR REPLACE FUNCTION obtener_reporte_brigadistas(IN p_norden integer, IN name_service text) RETURNS TABLE(
        ...
        fec_nac text -- Datos Brigadista (ip.*) 
,
        afron_tdd text,
        estilo_conflicto text,
        afron_sit_riesgo text,
        ...
        user_registro text,
        fecha_registro date,
        usuario_firma text -- Gestión y Sede 
,
        color integer,
        nombre_sede text,
        sede text,
...
(d.nombres_pa || ' ' || d.apellidos_pa)::text AS nombre,
    ...
    ) AS fec_nac -- Campos de psi_brigadistas 
,
    ip.afron_tdd,
    ip.estilo_conflicto,
    ip.afron_sit_riesgo,
...
values(
        'psi_brigadistas',
        'Formulario de Informe de Brigadistas',
        false,
        false,
        true,
        false,
...
CREATE OR REPLACE FUNCTION obtener_reporte_examen_complementario(IN p_norden integer, IN name_service text) RETURNS TABLE(
        ...
        edadpaciente text -- ORDEN / EMPRESA 
,
        norden integer,
        empresa text,
        cargopaciente text,
        ...
        codigoclinica text,
        tipoexamen text,
        fecha_apertura_po date -- RESULTADOS (EXAM_COMPLEMENTARIOS) 
,
        ...
        observacion text,
        recomenda text,
        cumple_perfil boolean -- AUDITORÍA Y FIRMA 
,
        user_registro text,
        fecha_registro date,
        usuario_firma text -- GESTIÓN Y SEDE (Corregido 'nombrespede' a 'nombresede') 
,
        nombresede text,
        sede text,
        color integer,
...
values(
        'cuestionario_berlin',
        'Formulario para Cuestionario de Berlin Psicología',
        false,
        false,
        true,
        false,
...
CREATE OR REPLACE FUNCTION obtener_reporte_cuestionario_berlin(IN p_norden integer, IN name_service text) RETURNS TABLE(
        dnipaciente integer,
        nombrespaciente text,
        apellidospaciente text,
        direccionpaciente text,
        ...
        codigoclinica text,
        tipoexamen text,
        edadpaciente text -- CAMPOS ESPECÍFICOS BERLIN 
,
...
END;
$BODY$ LANGUAGE plpgsql VOLATILE;
CREATE OR REPLACE FUNCTION obtener_reporte_trabajos_especificos(IN p_norden integer, IN name_service text) RETURNS TABLE(
        dnipaciente integer,
        nombrespaciente text,
        apellidospaciente text,
        direccionpaciente text,
...
values(
        'alto_riesgo',
        'Formulario para trabajos de alto riesgo',
        false,
        false,
        true,
        false,
...
CREATE OR REPLACE FUNCTION obtener_reporte_alto_riesgo(IN p_norden integer, IN name_service text) RETURNS TABLE(
        dnipaciente integer,
        nombrespaciente text,
        apellidospaciente text,
        direccionpaciente text,
...
,
    ...
    CAST(
        obtener_edad(d.fecha_nacimiento_pa, current_date) AS TEXT
    ) -- =  =  =  =  = DATOS DE ALTO RIESGO (Desde alto_riesgo - Tu nueva tabla) =  =  =  =  = 
,
...
END;
$BODY$ LANGUAGE plpgsql VOLATILE;
CREATE OR REPLACE FUNCTION obtener_reporte_informe_trastorno_personalidad(IN p_norden integer, IN name_service text) RETURNS TABLE(
        dnipaciente integer,
        nombrespaciente text,
        apellidospaciente text,
        direccionpaciente text,
...
END;
$BODY$ LANGUAGE plpgsql
CREATE OR REPLACE FUNCTION obtener_reporte_informe_conductores(IN p_norden integer, IN name_service text) RETURNS TABLE(
        dnipaciente integer,
        nombrespaciente text,
        apellidospaciente text,
        direccionpaciente text,
...
CREATE OR REPLACE FUNCTION obtener_reporte_informe_conductores(IN p_norden integer, IN name_service text) RETURNS TABLE (
        ...
        cargopaciente text,
        fechaapertura text,
        empresa text -- infor_conductores 
,
        crit_atencion text,
        crit_concetracion text,
        crit_segurid_control_conduc text,
...
SELECT d.cod_pa AS dnipaciente,
    ...
    n.razon_empresa AS empresa -- infor_conductores 
,
    ic.crit_atencion,
    ic.crit_concetracion,
    ic.crit_segurid_control_conduc,
...
END;
$BODY$;
CREATE OR REPLACE FUNCTION obtener_name_jasper(norden_param bigint, name_service_param text) RETURNS text AS $BODY$
...
ELSIF name_service_param = 'trastornos_personalidad' THEN resultado := 'Informe_Psico_Test_Personalidad';
ELSIF name_service_param = 'infor_conductores' THEN resultado := 'Informe_Psico_Conductores';
ELSIF name_service_param = 'certificacion_medica_altura' THEN resultado := 'A_CertificacionMedicaPTA_Digitalizado';
ELSIF name_service_param = 'alto_riesgo' THEN resultado := 'Informe_Psico_Alto_Riesgo';
ELSIF name_service_param = 'especificos' THEN resultado := 'Informe_Psico_Especificos';
...
CREATE OR REPLACE FUNCTION obtener_reporte_anexo16(IN p_norden integer, IN name_service text) RETURNS TABLE(
...
CASE
    ...
    t.diastolica,
CASE
        WHEN apt.ruido IS NULL THEN a.chkruido
        ELSE apt.ruido
    END,
CASE
        WHEN apt.polvo IS NULL THEN a.chkpolvo
        ELSE apt.polvo
    END,
CASE
        WHEN apt.vid_segmentario IS NULL THEN a.chkvidsegmentario
        ELSE apt.vid_segmentario
    END,
CASE
        WHEN apt.vid_total IS NULL THEN a.chkvidtotal
        ELSE apt.vid_total
    END,
CASE
        WHEN apt.cancerigenos IS NULL THEN a.chkcancerigenos
        ELSE apt.cancerigenos
    END,
CASE
        WHEN apt.mutagenicos IS NULL THEN a.chkmutagenicos
        ELSE apt.mutagenicos
    END,
CASE
        WHEN apt.solventes IS NULL THEN a.chksolventes
        ELSE apt.solventes
    END,
CASE
        WHEN apt.metales IS NULL THEN a.chkmetales
        ELSE apt.metales
    END,
CASE
        WHEN apt.temperatura IS NULL THEN a.chktemperatura
        ELSE apt.temperatura
    END,
CASE
        WHEN apt.biologicos IS NULL THEN a.chkbiologicos
        ELSE apt.biologicos
    END,
CASE
        WHEN apt.posturas IS NULL THEN a.chkposturas
        ELSE apt.posturas
    END,
CASE
        WHEN apt.turnos IS NULL THEN a.chkturnos
        ELSE apt.turnos
    END,
CASE
        WHEN apt.cargas IS NULL THEN a.chkcargas
        ELSE apt.cargas
    END,
CASE
        WHEN apt.mov_repet IS NULL THEN a.chkmovrepet
        ELSE apt.mov_repet
    END,
CASE
        WHEN apt.pvd IS NULL THEN a.chkpvd
        ELSE apt.pvd
    END,
CASE
        WHEN apt.electricos IS NULL THEN a.electricos
        ELSE apt.electricos
    END,
CASE
        WHEN apt.vibraciones IS NULL THEN a.vibraciones
        ELSE apt.vibraciones
    END,
CASE
        WHEN apt.otros IS NULL THEN a.chkotros
        ELSE apt.otros
    END,
CASE
        WHEN apt.altura_estructura IS NULL THEN a.altura_estructura
        ELSE apt.altura_estructura
    END,
CASE
        WHEN apt.altura_geografica IS NULL THEN a.altura_geog
        ELSE apt.altura_geografica
    END,
CASE
        WHEN apt.quimicos IS NULL THEN a.quimicos
        ELSE apt.quimicos
    END,
    a.tbrsi,
    a.rbrno,
...
CASE
    ...
    END AS o_i_8000,
CASE
        WHEN m.diagnostico is null THEN ''
        ELSE m.diagnostico
    END AS diagnostico,
...
CASE
        ...
        AND n.cod_sede <> 3 THEN 'Huamachuco'
        ELSE (CAST(sm.descripcion AS TEXT))
    END,
CASE
        WHEN UPPER(TRIM(n.razon_empresa)) = 'CIA MINERA PODEROSA S A' THEN 'Huamachuco'
...
CREATE OR REPLACE FUNCTION sp_validar_existencia_servicios(
    ...
    ) RETURNS TABLE(id_resp integer, mensaje text) AS $BODY$
...
begin if(p_examen_med = 'triaje') THEN
SELECT (
        CASE
            WHEN COUNT(*) > 0 THEN 1
            ELSE 0
...
LIMIT 1;
if(v_id_existencia = 0) THEN v_mensaje := 'SIN REGISTROS EN EL SISTEMA';
...
WHERE n_orden = p_historia_clinica
LIMIT 1;
if(v_triaje_existencia_espirometria = 1) THEN if(v_id_existencia = 0) THEN v_mensaje := 'SIN REGISTROS EN EL SISTEMA';
else v_mensaje := 'YA FUE REGISTRADO';
end if;
else v_mensaje := 'DEBE PASAR POR TRIAJE PRIMERO (OBLIGATORIO)';
v_id_existencia := 2;
end if;
end if;
...
if(
    p_examen_med = 'evaluacion_musculo_esqueletica2021'
) THEN
SELECT (
        CASE
            WHEN COUNT(*) > 0 THEN 1
...
SELECT CASE
        ...
        ELSE 0
    END INTO v_agudeza_visual_existencia;
IF(v_agudeza_visual_existencia = 1) THEN if(v_id_existencia = 0) THEN v_mensaje := 'SIN REGISTROS EN EL SISTEMA';
else v_mensaje := 'YA FUE REGISTRADO';
end if;
else v_mensaje := 'DEBE PASAR POR AGUDEZA VISUAL PRIMERO (OBLIGATORIO)';
v_id_existencia := 2;
end if;
end if;
...
WHERE n_orden = p_historia_clinica
LIMIT 1;
if(v_tabla_necesaria_existencia = 1) THEN if(v_id_existencia = 0) THEN v_mensaje := 'SIN REGISTROS EN EL SISTEMA';
else v_mensaje := 'YA FUE REGISTRADO';
end if;
else v_mensaje := 'DEBE PASAR POR ANEXO 2 PRIMERO (OBLIGATORIO)';
v_id_existencia := 2;
end if;
end if;
if(
    p_examen_med = 'certificado_aptitud_medico_ocupacional'
) THEN
SELECT (
        CASE
            WHEN COUNT(*) > 0 THEN 1
...
else v_mensaje := 'DEBE PASAR POR ANEXO 16 PRIMERO (OBLIGATORIO)';
v_id_existencia := 2;
end if;
end if;
...
if(
    p_examen_med = 'certificado_aptitud_medico_resumen'
) THEN
SELECT (
        CASE
            WHEN COUNT(*) > 0 THEN 1
...
else v_mensaje := 'DEBE PASAR POR FICHA SAS PRIMERO (OBLIGATORIO)';
v_id_existencia := 2;
end if;
end if;
...
SELECT CASE
        ...
        ELSE 0
    END INTO v_tabla_necesaria_existencia;
IF(v_tabla_necesaria_existencia = 1) THEN if(v_id_existencia = 0) THEN v_mensaje := 'SIN REGISTROS EN EL SISTEMA';
else v_mensaje := 'YA FUE REGISTRADO';
end if;
else v_mensaje := 'DEBE PASAR POR AUDIOMETRIA PRIMERO (OBLIGATORIO)';
v_id_existencia := 2;
end if;
end if;
...
if(
    p_examen_med = 'certificado_aptitud_herramientas_manuales'
) THEN
SELECT (
        CASE
            WHEN COUNT(*) > 0 THEN 1
...
if(
    p_examen_med = 'certificado_manipuladores_barrick'
) THEN
SELECT (
        CASE
            WHEN COUNT(*) > 0 THEN 1
...
if(p_examen_med = 'certificacion_medica_altura') THEN
SELECT (
        CASE
            WHEN COUNT(*) > 0 THEN 1
            ELSE 0
...
if(p_examen_med = 'alto_riesgo') THEN
SELECT (
        CASE
            WHEN COUNT(*) > 0 THEN 1
            ELSE 0
...
if(p_examen_med = 'especificos') THEN
SELECT (
        CASE
            WHEN COUNT(*) > 0 THEN 1
            ELSE 0
...
if(p_examen_med = 'cuestionario_berlin') THEN
SELECT (
        CASE
            WHEN COUNT(*) > 0 THEN 1
            ELSE 0
...
END IF;
END IF;
IF name_servicio_param = 'alto_riesgo' THEN IF(
    SELECT sello_prof_s
    FROM config_general_service_digital
    WHERE name_service = name_servicio_param
) THEN
...
END IF;
END IF;
IF name_servicio_param = 'certificacion_medica_altura' THEN IF(
    SELECT firma_p
    FROM config_general_service_digital
    WHERE name_service = name_servicio_param
) THEN descripcion := 'FIRMA DEL PACIENTE';
...
END IF;
END IF;
IF name_servicio_param = 'especificos' THEN IF(
    SELECT sello_prof_s
    FROM config_general_service_digital
    WHERE name_service = name_servicio_param
) THEN
...
END IF;
END IF;
IF name_servicio_param = 'cuestionario_berlin' THEN IF(
    SELECT sello_prof_s
    FROM config_general_service_digital
    WHERE name_service = name_servicio_param
) THEN
...
END IF;
END IF;
IF name_servicio_param = 'exam_complementarios' THEN IF(
    SELECT sello_prof_s
    FROM config_general_service_digital
    WHERE name_service = name_servicio_param
) THEN
...
END IF;
END IF;
IF name_servicio_param = 'psi_brigadistas' THEN IF(
    SELECT sello_prof_s
    FROM config_general_service_digital
    WHERE name_service = name_servicio_param
) THEN
